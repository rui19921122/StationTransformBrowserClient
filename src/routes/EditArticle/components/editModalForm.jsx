...
class ArticleForm extends React.Component {
    ...
    render() {
        ...
        return (this.props.edit_article.article && this.props.edit_article.article.id ?
            <div>
            ...
            })(<Input type="text" placeholder="标题" autoComplete={'off'}/>)}
                        </FormItem>
                ...
                initialValue: this.props.edit_article.article.menu
                        ...
                        <FormItem label="内容">
                            {getFieldDecorator('content', {})(<Editor initial_value={this.props.edit_article.article.content} key={this.props.article_id}/>)}
                        </FormItem>
                        <FormItem>
                            <Button type="primary" htmlType="submit" className="login-form-button" disabled={this.props.edit_article.article.content == this.props.form.getFieldValue('content') &&
                this.props.edit_article.article.name == this.props.form.getFieldValue('title')}>
                                提交
                            </Button>
                            <Upload action={`/api/article/${this.props.article_id}/files/`} showUploadList={false} onChange={(info) => {
                if (info.file.status == 'done') {
                    ...
                    <EditFileTable article_id={parseInt(this.props.article_id)} dispatch={this.props.dispatch} article={this.props.edit_article}/>
                </div>
            : <div>loading</div>);
    }
}
...
//# sourceMappingURL=editModalForm.jsx.map